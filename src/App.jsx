...
const VoidVoyager = () => {
  ...
  `,
    ...
    // Plasma function for realistic solar surface
    float plasma(vec3 p) {
      float result = 0.0;
      float speed = time * 0.2;
      
      // Granulation with different speeds and scales
      result += 0.5 * noise(p * 15.0 + vec3(speed * 0.5, 0.0, 0.0));
      result += 0.25 * noise(p * 30.0 + vec3(0.0, speed * 0.3, 0.0));
      result += 0.125 * noise(p * 60.0 + vec3(0.0, 0.0, speed * 0.2));
      result += 0.0625 * noise(p * 120.0 + vec3(speed * 0.1, speed * 0.1, 0.0));
      
      return result;
    }
    
    // Function to create sunspots
    float sunspot(vec3 p, vec3 center, float size) {
      float dist = length(p - center) / size;
      float spot = 1.0 - smoothstep(0.0, 1.0, dist);
      float edge = smoothstep(0.8, 1.0, dist) * smoothstep(1.0, 0.8, dist) * 2.0;
      return mix(spot * 0.5, edge, 0.5);
    }
    
    void main() {
      vec3 p = normalize(vPosition);
      
      // Base colors for solar spectrum
      vec3 baseColor = vec3(1.0, 0.9, 0.5);
      vec3 hotColor = vec3(1.0, 0.8, 0.3);
      vec3 coolColor = vec3(1.0, 0.3, 0.0);
      
      // Create granulation pattern (cellular-like structures)
      float plasmaNoise = plasma(p);
      
      // Create small scale granulation
      float smallGranules = noise(p * 80.0 + time * 0.1) * 0.1;
      
      // Limb darkening effect (darker at the edges)
      float limb = dot(vNormal, vec3(0.0, 0.0, 1.0));
      float limbDarkening = pow(limb, 0.5) * 0.4 + 0.6;
      
      // Create dynamic sunspots
      float spots = 0.0;
      
      // Large central spot group
      vec3 spotCenter1 = vec3(
        sin(time * 0.031) * 0.4,
        cos(time * 0.043) * 0.2,
        sin(time * 0.025) * 0.4
      );
      spots += sunspot(p, normalize(spotCenter1), 0.15);
      
      // Secondary spot group
      vec3 spotCenter2 = vec3(
        sin(time * 0.022 + 2.0) * 0.5,
        cos(time * 0.018 + 1.0) * 0.3,
        sin(time * 0.043 + 3.0) * 0.5
      );
      spots += sunspot(p, normalize(spotCenter2), 0.1);
      
      // Random smaller spots
      for (int i = 0; i < 3; i++) {
        float t = time * 0.01 + float(i) * 3.14;
        vec3 smallCenter = vec3(
          sin(t * 2.1 + float(i)) * 0.7,
          cos(t * 2.3 + float(i) * 1.5) * 0.4,
          sin(t * 1.5 + float(i) * 2.1) * 0.7
        );
        spots += sunspot(p, normalize(smallCenter), 0.05 + float(i) * 0.01);
      }
      
      // Solar flares
      float flares = 0.0;
      
      // Random flare positions
      for (int i = 0; i < 4; i++) {
        float t = time * 0.1 + float(i) * 1.618;
        vec3 flarePos = vec3(
          sin(t * 1.1 + float(i)) * 0.8,
          cos(t * 1.7 + float(i) * 0.5) * 0.4,
          sin(t * 1.3 + float(i) * 0.8) * 0.8
        );
        
        float flareDist = length(p - normalize(flarePos));
        float flareMask = smoothstep(0.12, 0.0, flareDist);
        
        float flareNoise = fbm(p * 5.0 + time * 0.2 + float(i));
        float flarePattern = smoothstep(0.2, 0.8, flareNoise) * flareMask;
        
        flares += flarePattern * (0.8 + 0.4 * sin(time * 2.0 + float(i)));
      }
      
      // Dynamic color mixing based on features
      float granuleIntensity = plasmaNoise + smallGranules;
      
      // Mix all surface features
      vec3 surfaceColor = mix(coolColor, hotColor, granuleIntensity);
      
      // Apply sunspots (darkening)
      surfaceColor = mix(surfaceColor, coolColor * 0.3, spots * 0.7);
      
      // Apply flares (brightening)
      surfaceColor = mix(surfaceColor, vec3(1.0, 0.9, 0.7), flares);
  ...
  `,
    ...
    void main() {
      // Normalize position for direction vector
      vec3 normalizedPos = normalize(vPosition);
      
      // Base intensity based on view angle
      float baseIntensity = pow(1.0 - abs(dot(vNormal, vec3(0.0, 0.0, 1.0))), 2.0);
      
      // Corona structure
      float time2 = time * 0.2;
      
      // Create wavy, filamentary structure with varying scales
      float coronaDetail = fbm(normalizedPos * 2.0 + vec3(time2 * 0.1));
      float coronaDetail2 = fbm(normalizedPos * 5.0 - vec3(0.0, time2 * 0.05, 0.0));
      float coronaDetail3 = fbm(normalizedPos * 10.0 + vec3(0.0, 0.0, time2 * 0.2));
      
      // Create larger corona structures using noise
      float largeStructure = noise(normalizedPos * 1.5 + vec3(time2 * 0.05)) * 0.5 + 0.5;
      
      // Mix details for dynamic filaments
      float coronaStructure = mix(
        coronaDetail * 0.6 + coronaDetail2 * 0.3 + coronaDetail3 * 0.1,
        largeStructure,
        0.4
      );
      
      // Create prominences (loop-like structures)
      ...
      for (int i = 0; i < 5; i++) {
        ...
        vec3 loopPos = vec3(
          sin(t + float(i)) * 0.8,
          cos(t * 0.7 + float(i) * 0.5) * 0.5,
          sin(t * 1.2 + float(i) * 1.7) * 0.8
        );
        
        // Loop shape
        float loopDist = length(normalizedPos - normalize(loopPos));
        float loopBase = smoothstep(0.4, 0.0, loopDist);
        
        // Loop texture
        float loopTex = fbm(normalizedPos * 8.0 + vec3(0.0, time * 0.05, 0.0));
        
        prominences += loopBase * loopTex * (1.5 + sin(time * 0.3 + float(i)) * 0.5);
      }
      ...
      float radialGradient = length(normalizedPos) / 16.0;
      vec3 coronaColor = mix(innerColor, midColor, radialGradient);
      coronaColor = mix(coronaColor, outerColor, radialGradient * radialGradient);
      
      // Add color variation based on fine structure
      coronaColor = mix(coronaColor, vec3(1.0, 0.8, 0.3), coronaStructure * 0.3);
      
      // Add prominences (brighter areas)
      coronaColor = mix(coronaColor, vec3(1.0, 0.9, 0.5), prominences * 0.7);
      
      // Final alpha based on corona structure and base intensity
      float alpha = baseIntensity * (coronaStructure * 0.5 + 0.1);
      
      // Add prominence alpha
      alpha += prominences * baseIntensity * 0.3;
      ...
      gl_FragColor = vec4(coronaColor * pulse, alpha);
    }
  `,
      transparent: true,
    ...
    const prominenceGeometry = new THREE.SphereGeometry(
      20,
      isLowPerformance ? 24 : 48,
      isLowPerformance ? 24 : 48
    );
  ...
  `,
    ...
    void main() {
      ...
      // Base intensity for outer edge
      float baseIntensity = pow(0.4 - dot(vNormal, vec3(0.0, 0.0, 1.0)), 4.0);
      
      // Create specific prominence locations
      float prominences = 0.0;
      for (int i = 0; i < 8; i++) {
        float t = time * 0.05 + float(i) * 0.785;
        
        // Position prominences around the sun
        vec3 prominencePos = vec3(
          sin(t) * 0.9,
          cos(t * 0.7) * 0.3 * hash(vec2(float(i), 1.0)),
          cos(t) * 0.9
        );
        
        float dist = length(normalizedPos - normalize(prominencePos));
        
        // Create arc/loop shape
        float arcShape = smoothstep(0.6, 0.0, dist);
        arcShape *= (0.5 + 0.5 * sin(time * 0.2 + float(i)));
        
        prominences = max(prominences, arcShape);
      }
      
      // Colors
      vec3 prominenceColor = vec3(1.0, 0.4, 0.0);
      
      // Final alpha
      float alpha = baseIntensity * prominences * 0.3;
  ...
  `,
    ...
    if (!isLowPerformance) {
      const flareGeometry = new THREE.SphereGeometry(12, 32, 32);
    ...
    `,
      ...
      void main() {
        ...
        // Create dynamic solar flares
        float flares = 0.0;
        
        // Random flare positions that move over time
        for (int i = 0; i < 3; i++) {
          float timeScale = time * 0.2 + float(i) * 1.0;
          float size = 0.1 + 0.1 * hash(vec3(float(i) * 10.0, 0.0, 0.0));
          
          // Position flares on solar surface
          vec3 flarePos = vec3(
            sin(timeScale * 0.3) * 0.9,
            cos(timeScale * 0.2) * 0.4,
            sin(timeScale * 0.5 + 2.0) * 0.9
          );
          
          float flareDist = length(normalizedPos - normalize(flarePos));
          
          // Create erupting pattern
          float eruptionTime = mod(time * 0.3 + float(i) * 3.0, 6.0);
          float eruptionPhase = clamp(eruptionTime - 1.0, 0.0, 4.0) * 0.25;
          
          float eruptionShape = smoothstep(size, size * 3.0, flareDist) * 
                               smoothstep(size * 5.0, size * 3.0, flareDist);
          
          float flareIntensity = eruptionShape * smoothstep(0.0, 1.0, eruptionPhase) * 
                                smoothstep(1.0, 0.0, eruptionPhase);
          
          flares = max(flares, flareIntensity);
        }
        
        // Flare color (bright yellow-white)
        vec3 flareColor = vec3(1.0, 0.9, 0.6);
        
        // Calculate alpha based on view angle and flare intensity
        float alpha = pow(0.5 - dot(vNormal, vec3(0.0, 0.0, 1.0)), 2.0) * flares * 0.7;
